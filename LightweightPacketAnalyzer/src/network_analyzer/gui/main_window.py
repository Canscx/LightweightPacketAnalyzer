...
会话数量: {info['session_count']}"""
    ...
    def _show_help(self) -> None:
        """显示帮助"""
        help_text = """网络流量统计系统 v2.0 使用说明:

【基本操作】
1. 数据包捕获
   • 点击"开始捕获"按钮或使用菜单"捕获→开始捕获"
   • 在数据包列表中实时查看捕获的网络数据包
   • 点击"停止捕获"按钮停止捕获

2. 数据包分析
   • 在数据包列表中点击任意数据包查看详细信息
   • 右侧面板显示协议树、十六进制数据和原始数据
   • 查看数据包的源地址、目标地址、协议类型等信息

【会话管理】
3. 会话操作
   • 新建会话：清空当前数据，开始新的分析会话
   • 打开会话：从数据库加载历史会话数据
   • 保存会话：将当前会话数据保存到数据库

【数据管理】
4. 数据导出
   • 使用"文件→导出数据"将数据包信息导出为CSV格式
   • 支持导出完整的数据包列表和统计信息
